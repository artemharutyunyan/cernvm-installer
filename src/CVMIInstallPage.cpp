...
void CVMIInstallPage::unzip()
{
  ...
  qDebug() << "_unzip_ Uncompressed image " << tmpUncompressedImage;

  QString gzipBin;  
  #ifdef Q_OS_WIN32
    gzipBin = installDir + "gzip.exe";
    ...
    process.start(gzipBin, QStringList() << "-d" << "--to-stdout" <<  imagePath);
  #else
    if (imagePath.contains(".vdi.gz"))
    {
      gzipBin = "gzip";
