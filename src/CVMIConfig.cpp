...
void CVMIConfig::reset()
{
  ...
  releaseSource = "http://rbuilder.cern.ch/installer/installer.cgi"; 
  ...
  out << "releaseSource = " << releaseSource << "\n";
  confFile.close();
}
...
void CVMIConfig::read()
{
  ...
  while (!confString.isNull())
  {
    ...
    if (rx.indexIn(confString) != -1 )
    {
      ...
      else if (var.compare("releaseSource") == 0)
      {
        releaseSource = val;
        qDebug() << "_conf_ read releaseSource" << releaseSource;
      }
    }
    confString = in.readLine();
  } 
...
QString CVMIConfig::getReleaseSource()
{
  if ( releaseSource.isEmpty())
    releaseSource = DEFAULT_RELEASE_SOURCE;
...
void CVMIConfig::setReleaseSource (QString source)
{
  qDebug() << "_conf_ releaseSource " << source;
