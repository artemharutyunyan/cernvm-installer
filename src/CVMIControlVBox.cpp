...
void CVMIControlVBox::createVM()
{
  ...
  qDebug() << "_VBox_ createvm Looking for the image in: " << *uncompressedImagePath << ""; 

  // Try to see if we have a .vdi file in the directory
  QStringList filter;
  ...
  {
    vmImageFullPath = info.absoluteFilePath();  
    qDebug() << "_VBox_ createvm the image is ready: " << info.absoluteFilePath();

    if (vmImageFullPath.contains("x86_64"))
      vmOsType += "_64";
  }   

  if (vmImageFullPath.isEmpty()) // Could not find .vdi, now try with vmdk
  {
    filter.clear();
    filter << "cernvm-*";
    dir.setNameFilters (filter);
